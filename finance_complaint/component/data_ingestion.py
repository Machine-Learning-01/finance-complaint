...
class DataIngestion:
    ...
    def convert_files_into_csv(self):
        try:
            data_dir = self.data_ingestion_config.csv_data_dir
            os.makedirs(data_dir,exist_ok=True)
            file_path = os.path.join(data_dir,self.data_ingestion_config.file_name)
            for file_name  in os.listdir(self.data_ingestion_config.download_dir):
                pd.read_json(os.path.join(data_dir,file_name)).to_csv(file_path,mode="a+",header=None,index=None)
        except Exception as e:
            raise FinanceException(e,sys)
    ...
    def retry_download_data(self,data,download_url:DownloadUrl):
        try:
                ...
                if download_url.n_retry==0:
                    self.failed_download_urls(download_url)
                ...
                failed_file_path = os.path.join(os.path.dirname(download_url.file_path),"failed_response",os.path.basename(download_url.file_path))
                os.makedirs(failed_file_path,exist_ok=True)
...
def main():
    try:
        ...
        data_ingestion = DataIngestion(data_ingestion_config=data_ingestion_config, n_month_interval=3)
